...
const html=fs.readFileSync('index.html','utf8'),sw=fs.readFileSync('service-worker.js','utf8'),css=fs.readFileSync('style.css','utf8');
...
const theme=/name="theme-color" content="([^"]+)"/.exec(html)?.[1],background=/--bg:\s*(#[0-9a-f]{6})/i.exec(css)?.[1];
assert.equal(theme,manifest.theme_color,'HTML theme color must match manifest');assert.equal(background,manifest.background_color,'CSS background must match manifest');
...
console.log('Syntax, local assets, unique IDs, PWA palette, manifest dimensions and CSP-compatible scripts verified.');
